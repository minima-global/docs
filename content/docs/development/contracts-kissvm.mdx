...
| SETLEN ( NUMBER HEX ) | Set the length of a HEX value, trims if too large or pads with 0 if too short |
...
| PROOF ( HEX \|STRING NUMBER HEX NUMBER HEX ) | Check the mmr data, mmr root, with mmr proof are correct. Use 0 Numbers for non-sum mmr trees. Same as mmrproof on Minima |
